function [Gbest,T] = PCSO( fhd,Dimension,Particle_Number,Mr,srd,smp,c,Max_Gen,VelMin,VelMax,popmin,popmax,varargin )
%CSO 此处显示有关此函数的摘要
%   此处显示详细说明CSO
%-------------------------- Parameters Setting ----------------------------%
...
%             [group(g).max,group(g).worseposition] = max(group(g).fitness);
        ...
        for m = 1:size(idx_Trace,2)
            if rem(G,20) == 0%R1=20,20代交流一次，
               ...
               group(4).pop(group(4).worseposition4,:) = group(r).pop(idx_Trace(m),:);
%               group(4).pop(r,:) = group(r).pop(idx_Trace(m),:);
            end
            ...
            %-------------------------------------------------------------------%
             ...
             [group(1).max,group(1).worseposition1] = max(group(1).pop(idx_Trace(m),d) );
               [group(2).max,group(2).worseposition2] = max(group(2).pop(idx_Trace(m),d) );
                [group(3).max,group(3).worseposition3] = max(group(3).pop(idx_Trace(m),d) );
                 [group(4).max,group(4).worseposition4] = max(group(4).pop(idx_Trace(m),d) );
        end
