function [Gbest,T] = PCCSO2( fhd,Dimension,Particle_Number,Mr,srd,smp,c,Max_Gen,VelMin,VelMax,popmin,popmax,varargin )
%CSO 此处显示有关此函数的摘要
%   此处显示详细说明CSO
%-------------------------- Parameters Setting ----------------------------%
...
%                 r = randperm(size(group,2),1);
%             end
            A=(group(1).gbest+group(2).gbest+group(3).gbest+group(4).gbest)/4;
