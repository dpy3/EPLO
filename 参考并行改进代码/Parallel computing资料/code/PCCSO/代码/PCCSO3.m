function [Gbest,T] = PCCSO3( fhd,Dimension,Particle_Number,Mr,srd,smp,c,Max_Gen,VelMin,VelMax,popmin,popmax,varargin )
%CSO 此处显示有关此函数的摘要
%   此处显示详细说明CSO
%-------------------------- Parameters Setting ----------------------------%
...
%                 r = randperm(size(group,2),1);
%             end
               q = randperm(size(nPOP ,2),1);
        
            A=(0.1*group(1).gbest+0.2*group(2).gbest+0.3*group(3).gbest+0.4*group(4).gbest)/1;
 %            group(g).pop(group(g).worseposition,:) = A;
             group(g).pop(q,:) = A;
