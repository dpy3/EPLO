% function [fitnessgbest,T] = APSO( fhd,Dimension,Particle_Number,Max_Gen,VRmin,VRmax,Popmin,Popmax,varargin )
%APSO 此处显示有关此函数的摘要
%   此处显示详细说明
function [fitnessgbest,T] =APSO(opts,CostFunction)
c1 = 2.0;
c2 = 2.0;
w = 0.9;
...
%         Jump = gbest(Rd)+Z;
        ...
        Ju(Rd) = Jump;
%         a= CostFunction(Ju(j,:));
           Jumpfitness(1,:) = CostFunction(Ju(1,:));
%         Jumpfitness = feval(fhd,Ju',varargin{:});
...
% figure(varargin{:});
% plot(T(1:50:1000),'m-','LineWidth',1,'Marker','v','MarkerFaceColor','m');
% hold on;
% legend('PSO','PPSO','CCPSO','APSO');
